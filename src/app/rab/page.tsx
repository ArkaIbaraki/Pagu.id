...
import { RabForm } from '@/components/rab-form';
import { RabPreview } from '@/components/rab-preview';

export default function RabPage() {
  const [mounted, setMounted] = useState(false);
  const isDark = useInvoiceStore((state) => state.isDark);
  const setDarkMode = useInvoiceStore((state) => state.setDarkMode);
  const history = useRabStore((state) => state.history);
  const loadFromHistory = useRabStore((state) => state.loadFromHistory);
  const clearHistory = useRabStore((state) => state.clearHistory);
                ...
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  <div className="p-3 max-h-64 overflow-y-auto">
                    {history.map((item, idx) => (
                      ...
                      >
                        <div className="font-semibold">{item.nomorRab}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {item.namaProyek || 'No Name'}
                        </div>
                      </button>
                    ))}
                    <button
