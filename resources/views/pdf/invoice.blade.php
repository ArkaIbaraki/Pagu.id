...
<body>
    ...
    <table style="margin-bottom: 0;">
        <thead>
            <tr style="border-bottom: 2px solid #000;">
                ...
                @if ($modeQuantity)
                    <th style="text-align: center; padding: 8px; width: 10%; font-weight: bold; font-size: 11px;">Qty
                    </th>
                    <th style="text-align: right; padding: 8px; width: 20%; font-weight: bold; font-size: 11px;">Harga
                    </th>
                @endif
                <th style="text-align: right; padding: 8px; width: 20%; font-weight: bold; font-size: 11px;">Jumlah (Rp)
                </th>
            </tr>
        </thead>
        <tbody>
            @forelse($items as $idx => $item)
                <tr style="border-bottom: 1px solid #ccc;">
                    ...
                    @if ($modeQuantity)
                        ...
                        <td style="text-align: right; padding: 8px; font-size: 11px;">Rp
                            {{ number_format((float) ($item['harga'] ?? 0), 0, ',', '.') }}</td>
                    @endif
                    <td style="text-align: right; padding: 8px; font-size: 11px;">Rp
                        {{ number_format((float) ($modeQuantity ? $item['qty'] ?? 1 : 1) * (float) ($item['harga'] ?? 0), 0, ',', '.') }}
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="{{ $modeQuantity ? 5 : 3 }}" style="text-align: center; padding: 8px; color: #999;">
                        Belum ada item</td>
                </tr>
            @endforelse
        </tbody>
        ...
        <tbody>
            <tr style="border-bottom: 1px solid #ccc;">
                <td style="padding: 5px 8px; text-align: right; font-size: 11px;"
                    colspan="{{ $modeQuantity ? 3 : 1 }}"></td>
                <td style="padding: 5px 8px; text-align: right; font-weight: bold; font-size: 11px;">Subtotal</td>
                <td style="padding: 5px 8px; text-align: right; font-size: 11px; width: 15%;">Rp
            ...
            @if ($useDiskon && $diskonValue > 0)
                <tr style="border-bottom: 1px solid #ccc;">
                    ...
                    <td style="padding: 5px 8px; text-align: right; font-size: 11px;">Rp
                        {{ number_format($diskonValue, 0, ',', '.') }}</td>
                </tr>
            @endif
            @if ($usePpn)
                <tr style="border-bottom: 1px solid #ccc;">
                    ...
                    <td style="padding: 5px 8px; text-align: right; font-size: 11px;">Rp
                        {{ number_format($ppnValue, 0, ',', '.') }}</td>
                </tr>
            @endif
            <tr style="border-top: 2px solid #000; border-bottom: 2px solid #000;">
                <td style="padding: 8px; text-align: right; font-size: 11px;" colspan="{{ $modeQuantity ? 3 : 1 }}">
                </td>
                <td style="padding: 8px; text-align: right; font-weight: bold; font-size: 12px;">Total</td>
                <td style="padding: 8px; text-align: right; font-weight: bold; font-size: 12px;">Rp
    ...
    @if ($namaTtd)
        <div style="margin-top: 40px; text-align: right;">
            ...
            <div
                style="font-size: 11px; font-weight: bold; border-top: 1px solid #333; display: inline-block; padding-top: 5px; padding-left: 20px; padding-right: 20px;">
                {{ $namaTtd }}</div>
        </div>
    @endif
</body>
